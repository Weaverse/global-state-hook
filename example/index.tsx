...
const useCounter = () => {
  let { state, subscription } = useSyncStore(counterSubscription)
  const increment = () => subscription.updateState({ count: state.count + 1 })
  const decrement = () => subscription.updateState({ count: state.count - 1 })
...
let CounterDisplay2 = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => subscription.updateState({ count: 0 })}>
        Reset
      </button>
    </div>
  )
...
function FooDisplay() {
  ...
  return (
    <div>
      Only update when foo change
      <button onClick={() => subscription.updateState({ foo: state.foo - 1 })}>
        -
      </button>
      <span>{state.foo}</span>
      <button onClick={() => subscription.updateState({ foo: state.foo + 1 })}>
